...
fn field_offsets(data: &[u8]) -> Vec<(usize, usize)> {
...
fn metadata_value_slots(data: &[u8]) -> Vec<usize> {
...
fn mutate_raw(seed: &[u8], rng: &mut StdRng) -> Vec<u8> {
...
/// (25 KiB valid models).
fn mutate_construction(seed: &[u8], rng: &mut StdRng) -> Vec<u8> {
    let mut data: Vec<u8> = seed.to_vec();
...
/// patches on the valid models, reaching model construction). `clap`'s
/// `ValueEnum` is derived here so the CLI and the Python binding share one
/// enum.
...
impl CorpusMode {
    /// Machine name used in run tags, summaries, and errors.
    ...
    /// Parse a mode name (`raw`, `construction`), case-insensitively.
...
/// Run tag shared by the JSONL log and summary filenames.
fn corpus_run_tag(mode: CorpusMode, n: usize, seed: u64) -> String {
    format!("{}-{n}-{seed}", mode.as_str())
}
...
/// Run summary written to `summary_{mode}-{n}-{seed}.json`. First seven keys
/// mirror diff_fuzz.py's `summary_{mode}-{n}-{seed}.json`; `runner`/`rng_note`
/// are additive provenance for the audit trail.
#[derive(Debug, Clone, Serialize)]
...
/// Aggregate per-target outcome counts and counters for one campaign.
#[derive(Debug, Clone, Serialize)]
pub struct CorpusRunStats {
    pub per_target: BTreeMap<String, BTreeMap<String, u64>>,
    pub failure_inputs_saved: u64,
    /// Cases whose evaluation itself errored (logged, never silently dropped).
    pub failed_cases: u64,
    pub elapsed_s: f64,
    /// JSONL log written by the run.
...
struct CorpusRunConfig {
    ...
    seed: u64,
    mode: CorpusMode,
    seed_blobs: Vec<Vec<u8>>,
    against: Vec<ExternalRuntime>,
    timeout: Duration,
    jobs: usize,
    out_dir: PathBuf,
    /// Print the CLI's header/progress lines (the Python binding passes false).
    verbose: bool,
}
...
/// the caller.
fn run_corpus(config: &CorpusRunConfig) -> anyhow::Result<CorpusRunStats> {
        ...
        for _ in 0..jobs {
            scope.spawn(|| loop {
                ...
                if let Err(e) = eval_one(i, config, &externals, &shared) {
                    shared.failed_cases.fetch_add(1, Ordering::SeqCst);
                    if config.verbose {
                        eprintln!("diff-corpus: case {i} evaluation failed: {e:#}");
                    }
                }
                let done = shared.completed.fetch_add(1, Ordering::SeqCst) + 1;
                if config.verbose && (done.is_multiple_of(500) || done == config.n) {
    ...
    Ok(CorpusRunStats {
        ...
        failed_cases: shared.failed_cases.load(Ordering::SeqCst),
        elapsed_s,
        log_path,
    })
}
...
/// Fully resolved corpus campaign request (`ember diff-corpus`).
#[derive(Debug, Clone)]
pub struct CorpusRequest {
    /// Number of mutations to generate and evaluate.
    pub n: usize,
    /// RNG seed (`StdRng`: same operators/slots/tables as `diff_fuzz.py`, but
    /// the stream is NOT bit-identical to Python's `random.Random`).
    pub seed: u64,
    /// Mutation mode.
    pub mode: CorpusMode,
    /// External runtimes to compare against; ember always runs in-process.
    pub against: Vec<ExternalRuntime>,
    /// Per-runtime deadline in seconds (externals; ember is in-process).
    pub timeout_secs: f64,
    /// Parallel case workers; clamped to at most 4 concurrent child processes.
    pub jobs: usize,
    /// Output directory (REFUSED when inside `research/embersec/comparative/`).
    pub out_dir: PathBuf,
    /// Explicit seed files; empty selects the default mode-specific set.
    pub seeds: Vec<String>,
}

/// Completed corpus campaign: summary, artifact paths, and per-target counts.
#[derive(Debug, Clone, Serialize)]
pub struct CorpusOutcome {
    ...
    pub stats: CorpusRunStats,
}

/// Run a full campaign: resolve seeds, guard the out-dir, stream mutations,
/// evaluate every case, write the JSONL log and
/// `summary_{mode}-{n}-{seed}.json`, and return the summary plus artifact
/// paths.
///
/// `verbose` prints the CLI's header, 500-case checkpoints, per-target
/// totals, and case-failure lines; the Python binding passes `false`
/// (failures are still counted in [`CorpusRunStats::failed_cases`]).
pub fn run_diff_corpus(request: &CorpusRequest, verbose: bool) -> anyhow::Result<CorpusOutcome> {
    anyhow::ensure!(request.n > 0, "n must be positive");
    anyhow::ensure!(request.jobs > 0, "jobs must be positive");
    anyhow::ensure!(
    ...
    let (seed_names, seed_blobs) = if request.seeds.is_empty() {
        ...
        load_override_seeds(&request.seeds)?
    };
    ...
    let run_tag = corpus_run_tag(request.mode, request.n, request.seed);
    let summary = CorpusSummary {
        mode: request.mode.as_str().to_string(),
        n_mutations: request.n,
        seed_cases: seed_names.len(),
        rng_seed: request.seed,
        timeout_s: request.timeout_secs,
...
mod tests {
    ...
    #[test]
    fn corpus_mode_parses_known_names_only() {
        assert_eq!(CorpusMode::parse("raw"), Some(CorpusMode::Raw));
        assert_eq!(
            CorpusMode::parse(" CONSTRUCTION "),
            Some(CorpusMode::Construction)
        );
    ...
    #[test]
    fn campaign_writes_summary_and_reports_paths() {
        let dir = std::env::temp_dir().join(format!("ember-campaign-{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
        let seed_a = dir.join("seed_a.bin");
        let seed_b = dir.join("seed_b.bin");
        std::fs::write(&seed_a, tiny_header(0, 0)).unwrap();
        std::fs::write(&seed_b, b"definitely not gguf........").unwrap();
        let request = CorpusRequest {
            ...
            seeds: vec![seed_a.display().to_string(), seed_b.display().to_string()],
        };
        let outcome = run_diff_corpus(&request, false).unwrap();
        assert_eq!(outcome.run_tag, "raw-2-7");
        assert_eq!(outcome.summary.n_mutations, 2);
        assert_eq!(outcome.summary.seed_cases, 2);
        assert_eq!(outcome.stats.failed_cases, 0);
        assert!(outcome.summary_path.is_file());
        let written: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&outcome.summary_path).unwrap()).unwrap();
        assert_eq!(written["n_mutations"], 2);
        // One target (ember) x two cases.
        let log = std::fs::read_to_string(&outcome.log_path).unwrap();
        assert_eq!(log.lines().count(), 2);
        assert!(outcome.stats.per_target.contains_key("ember"));
        // Invalid requests are refused before any work.
        let zero = CorpusRequest {
            n: 0,
            ..request.clone()
        };
        assert!(run_diff_corpus(&zero, false).is_err());
        std::fs::remove_dir_all(&dir).unwrap();
    }
}
