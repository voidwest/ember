fn main() {
    ...
    let up = ember::multimodal::image::resize(&img, 2048, 2048, ember::multimodal::image::Resample::Bicubic).unwrap();
    let down = ember::multimodal::image::resize(&up, 512, 512, ember::multimodal::image::Resample::Bicubic).unwrap();
    // normalized pixels like the processor: (x/255 - 0.5)/0.5
    let mut outv = Vec::with_capacity(3*512*512);
    for v in down.data() { outv.push((v/255.0 - 0.5)/0.5); }
