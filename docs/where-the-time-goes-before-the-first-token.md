...
The packed-weight cache reduced Ember's warm Llama-3.2-1B Q8_0 build time. It fell from 670 ms to 83–84 ms. Decode throughput stayed within measurement noise; the gain is in warm construction.
...
Thermal throttling made small throughput deltas
...
Figure 1. Lower is better.

Each panel has its own millisecond scale. Model
build uses the September 12 cache measurements: cache-off median 670 ms;
first-write samples 746/749/751 ms; warm-hit samples 83/84/83 ms.
...
An earlier profile attributed 478 ms of a 663 ms model build to VNNI
...
to in-memory packing.

Tests cover byte identity, invalidation, corruption,
...
2.06–2.28 seconds to 1.54–1.60 seconds.

Packing accounts for less of that
...
Gemma in a separate comparison.

These wall-time measurements use different
...
configuration is unchanged and made the caller reuse its logits buffer. The planned Q4 path went from five allocation events and 515,716 allocated
bytes per token to two events and 3,040 bytes.
...
That larger comparison combines the default change with buffer
