...
\item Evidence that POS survives heldout evaluation with consistent lift over surface baselines in both models tested (Qwen3-0.6B: +19.2\%; Llama-3.2-1B: +15.2\%), with results strengthening from 1.4k to 5k scale. Gender and number show modest positive lifts.
...
\item A reusable pipeline for multi-model heldout probing of Arabic morphology. Code and data at \url{https://github.com/voidwest/ember}.
\end{enumerate}
...
\textbf{Arabic morphology probing.} \citet{abdelali-etal-2022} probed Arabic transformer models (AraBERT, CAMeLBERT) across layers, finding that lower and middle layers capture word morphology while higher layers capture syntactic dependencies, but focused on POS and morphological tagging rather than root/pattern identification and did not employ lexical heldout controls. Their evaluation used standard random splits, which our analysis suggests would inflate accuracy for high-cardinality labels through root/lemma overlap. \citet{shapiro-etal-2021} proposed multilabel morphosyntactic probing for multilingual embeddings, including Arabic as a held-out zero-shot language for features such as gender and number, but did not evaluate under Arabic-specific lexical heldout splits. We extend this line of work with (a) Arabic root, lemma, abstract-pattern, and concrete-pattern probing, (b) leakage-aware GroupKFold by lemma and root with explicit closed-set verification---showing that high-cardinality labels produce unseen test labels in every fold, and (c) cross-model replication across Qwen3-0.6B and Llama-3.2-1B architectures with fundamentally different tokenization.
...
Under standard stratified cross-validation on the 1,416-token pilot (all three models, including Qwen2.5), all seven tasks showed high probe accuracy and strong lift over surface baselines. Root achieved 93.6\% (164 classes, +86.0\% over majority), lemma 97.2\%, abstract pattern 93.5\%, POS 94.6\%. All tasks passed shuffled-label controls with selectivity $>$ 0.89. Multi-seed stability was tight (std $\leq$ 0.5\%). At 5k scale with Qwen3-0.6B (Ridge $\alpha=1.0$), POS random CV reaches 93.4\%; higher-cardinality tasks were not re-run under random CV at 5k since the heldout evaluation is the primary methodology.

The leakage audit reveals that 87.9\% of roots appear multiple times in the dataset. Under random CV, the same root can appear in both training and test folds. These results likely reflect lexical co-occurrence rather than morphological generalization. We report random-CV results only as an optimistic baseline and to quantify how conclusions change under heldout validation.
...
\textbf{POS} is the strongest validated finding. Under lemma-heldout at 5k scale, Qwen3-0.6B achieves 88.5\% (+19.2\% over the char n-gram baseline) at layer 8, while Llama-3.2-1B achieves 84.6\% (+15.2\%) at layer 0. The lift is consistent across root-heldout as well (Qwen3: 85.5\% +14.4\%, Llama: 82.9\% +11.8\%). At 1.4k scale, the same method gave Qwen3 86.0\% (+16.5\%) and Llama 84.0\% (+14.5\%); the 5k expansion produced modest accuracy gains (+2.5pp Qwen3, +0.6pp Llama) with wider probe--char gaps.
...
POS survives heldout evaluation with positive lift over the char baseline in both models tested. The lift \textit{increases} as the split becomes stricter: at 5k scale, Qwen3-0.6B achieves +2.2\% under random CV, +19.2\% under lemma-heldout, and +14.4\% under root-heldout. This pattern is consistent with the interpretation that the char baseline relies on surface-form memorization (degrading sharply when lemmas or roots are held out), while the probe accesses representations that generalize across lemmas. This result replicates in both architectures despite substantially different Arabic tokenization (Qwen3: 15.9\% single-token; Llama: 0\%) and layer counts. At 1.4k scale, the same method gave Qwen3 86.0\% (+16.5\%) and Llama 84.0\% (+14.5\%); the 5k expansion produced modest accuracy gains with wider probe--char gaps.

\subsection{Gender and Number: Positive but Modest}
At 5k scale, gender and number show positive lifts under lemma-heldout: Qwen3 gender 95.3\% (+12.3\%), number 94.0\% (+5.4\%); Llama gender 95.5\% (+4.4\%), number 94.0\% (+4.6\%). These features are heavily surface-marked in Arabic---the char n-gram baseline already achieves high accuracy (83--91\%)---so the residual probe lift is smaller than for POS. We note an asymmetry in gender lift between models (+12.3\% Qwen3 vs.\ +4.4\% Llama) that we did not investigate further; it may reflect the same architectural divergence seen in POS layerwise patterns, or may be an artifact of the 2048-dim Llama embedding space interacting differently with the period-token aggregation position.
...
\item \textbf{Final-token pooling:} We extract hidden states from the final token of the prompt template (the period), not from the final subword token of the Arabic stimulus. This probes the model's aggregate representation of the full prompt at the last position. Moreover, the prompt itself (Appendix~\ref{sec:appendix-prompt}) includes the root, lemma, and pattern of the stimulus word---information the model has processed before reaching the period token. The period's hidden state may therefore encode a task-setup representation (the model has been given morphological information and asked to predict) rather than intrinsic morphological encoding of the stimulus. Ablations without morphological information in the prompt, or probing at the stimulus tokens themselves, would be needed to distinguish these interpretations.
