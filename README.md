...
`_metadata.json` sidecars with predicted-vs-expected continuations and the
exact prompt template, position, model, shape, and token selections used.
...
| script | purpose |
|--------|---------|
| `train_linear_probe.py` | linear probes with task-specific CV splits, control tasks, and selectivity |
