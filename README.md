...
- **probing pipeline**: python scripts for linear probes (with control tasks and
  selectivity), CCA, RSA, divergence analysis, cross-model comparison plots, and
...
per-layer hidden states at the last token position. saves a 3d `.npy` array
`(n_stimuli, n_layers, embed_dim)` plus a `_correctness.json` file with
predicted-vs-expected continuations. works with both gpt-2 and llama
architectures through the `ForwardModel` trait.

the `probes/` directory contains python scripts for downstream analysis:

| script | purpose |
|--------|---------|
| `train_linear_probe.py` | linear probes with CV, control tasks, selectivity, root-disjoint splits |
...
stimuli: 20 roots × 10 patterns, from Alakeel et al. 2026).
...
- **Llama 3 tokenizer is balanced for Arabic**: ar/en token ratio is 1.2×
  vs 2.4× for GPT-2's tokenizer on the same prompts.

full experiment log: [EXP.md](EXP.md)
